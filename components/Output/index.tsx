...
function Index({ data }: { data: any }) {
  ...
  const downloadFile = () => {
    download(data.data, `${data.fileName.trim()}.sol`, "text/plain");
  };
  ...
  return (
    ...
    >
      <div className="shadow-lg bg-blue_1 w-11/12 rounded-lg overflow-hidden max-w-lg lg:max-w-2xl">
        <nav className=" bg-dark flex items-center justify-between py-4 px-6 shadow_custom">
          <h2 className=" font-bold py-2 px-4 bg-blue_1 shadow text-dark rounded-lg shadow_custom">
            {data.fileName}.sol
          </h2>
          <h3>
            <button
              className="bg-blue_1 duration-75 transform hover:scale-105 active:scale-95 font-bold py-2 px-4 rounded-lg shadow_custom"
